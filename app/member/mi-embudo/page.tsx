...
export default function MemberEmbudoPage() {
  ...
  const handleShareWhatsApp = (embudoId: string) => {
    const activeE = EMBUDOS.find(e => e.id === embudoId)
    const url = `${window.location.origin}/r/${user?.username || finalMember.id}/${embudoId}`
    const text = activeE?.persuasiveText
      ? `${activeE.persuasiveText}\n${url}`
      : `🚀 Mira el nuevo embudo: *${activeE?.nombre}*\n${url}`
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={cn(
                      "flex flex-1 items-center justify-center gap-1.5 rounded-lg px-3 py-2.5 text-xs font-medium transition-all sm:flex-initial",
                  ...
                  >
                    ...
                    {copied === activeEmbudo.id ? "Copiado" : "Copiar Link"}
                  </button>
                  <button
                    onClick={() => handleShareWhatsApp(activeEmbudo.id)}
                    className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-emerald-500/10 px-3 py-2.5 text-xs font-medium text-emerald-400 transition-all hover:bg-emerald-500/20 sm:flex-initial"
                  >
